 ...
 */
//@Configuration
public class WebSecurityAccessConfig extends WebSecurityConfigurerAdapter {
    @Autowired
...
//        http.authorizeRequests().antMatchers("/user/login","/login.html","/error.html").permitAll();
        ...
        http.authorizeRequests().antMatchers("/user/login","/login.html","/error.html").access("permitAll()");
        //如果用户具备给某个权限admin，就允许访问。
        http.authorizeRequests().antMatchers("/Authority/haveAuthority").access("hasAnyAuthority('admin')");
        //如果用户具备给某个权限user，就允许访问。
        http.authorizeRequests().antMatchers("/Authority/notAuthority").access("hasAuthority('user')");
        //如果用户具备给定权限中某一个，就允许访问。
        http.authorizeRequests().antMatchers("/Authority/hasAnyAuthority").access("hasAnyAuthority('user','admin')");



        //如果用户具备给某个权限admin，就允许访问。
        http.authorizeRequests().antMatchers("/Permission/havePermission").access("@accessSecurityExpressionOperations.hasPermission(request)");
