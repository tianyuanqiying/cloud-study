 ...
 */
//@Configuration
public class WebSecurityRoleConfig extends WebSecurityConfigurerAdapter {
    @Autowired
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        auth.userDetailsService(new UserDetailsService() {
            ...
            public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
                ArrayList<SimpleGrantedAuthority> permission = CollectionUtil.newArrayList(new SimpleGrantedAuthority("ROLE_admin"), new SimpleGrantedAuthority("user"));
...
//        http.authorizeRequests().antMatchers("/user/login","/login.html","/error.html").permitAll();
        ...
        http.authorizeRequests().antMatchers("/user/login","/login.html","/error.html").permitAll();
        //如果用户具备给某个角色Role_admin，就允许访问。
        http.authorizeRequests().antMatchers("/role/have").hasRole("admin");
        //如果用户具备给某个角色Role_user，就允许访问。
        http.authorizeRequests().antMatchers("/role/not").hasRole("user");
        //如果用户具备给定权限中某一个角色Role_user， Role_admin，就允许访问。
        http.authorizeRequests().antMatchers("/role/hasAny").hasAnyRole("user","admin");
        //非登录的URL需要认证
